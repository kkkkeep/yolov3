...
def train(net, loss, optimizer, data, device, epochs):
    ...
    for epoch in range(0, epochs):
        ...
        for img, label in data:
            ...
            pre = net(img)
            loss_13 = loss(pre[0].float(), label[0].float(), c=0.3)
            loss_26 = loss(pre[1].float(), label[1].float(), c=0.3)
            loss_52 = loss(pre[2].float(), label[2].float(), c=0.3)
...
if __name__ == '__main__':
    ...
    train(net, loss, optimizer, data, args.device, epochs=100)
